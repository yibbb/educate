                    ...
                    <table class="layui-table layui-form">
                        ...
                        <c:forEach items="${studentClasses}" var="studentClass">
                            <tr>
                                    ...
                                    <%--                                    <input id="${studentClass.id}" value="${studentClass.id}">--%>
                                    <%--                                </td>--%>
                                <td>${studentClass.studentName}</td>
                                <td>${studentClass.courseName}</td>
                                <td>${studentClass.teacherName}</td>
                                <c:if test="${studentClass.isPaid==1}">
                                    <td>已支付</td>
                                </c:if>
                                <c:if test="${studentClass.isPaid==0}">
                                    <td>未支付</td>
                                </c:if>
                                <td>
                                    <button class="layui-btn"
                                        <%--                                            onclick="submit(this, $('#${studentClass.id}').val())"--%>
                                            onclick="submit(this, '${studentClass.id}')"
                                            lay-filter="confirm" lay-submit>
                                        确认
                                    </button>
                                </td>
                            </tr>
